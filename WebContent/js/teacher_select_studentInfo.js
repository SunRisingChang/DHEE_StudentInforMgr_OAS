...
$(document).ready(function() {
	...
	but1.click(function() {
		if (a.val() === ""&&b.val() === "") {
			...
			$.ajax({
				url : 'teacher_selectStudentInfo.action',
				type : 'POST',
				data : {
					"className" : a.val(),
					"studentName":b.val(),
				...
				success : function(data, textStatus, jqXHR) {
				    if(data==110){
						...
						$.each(data[0].pagelist,function(index,value) {
							$("#table1 tbody").append('<tr><td>'+(data[0].nowpage*10+1+index)+'</td><td>'+value.name+'</td><td>'+value.classes+'</td><td>'+((value.sex=="TT")?"男":"女")+'</td><td>'+value.college+'</td><td>'+value.major+'</td><td>'+value.educationBg+'</td><td>'+value.phone+'</td><td>'+value.employmentStatus+'</td><td style="text-align:center;"><q id="xiangqing" class="q1" rowid="'+index+'">详情</q><q id="shanchu" class="q2" rowid="'+value.id+'">删除</q></td></tr>');	
						})
						src_json=data;
	...
	$("#table1").on("click","#shanchu",function(){
		...
		layer.confirm('您确认要删除该条记录？', {
			  btn: ['是的','不是'] //按钮
				...
				$.ajax({
					url : 'teacher_deleteStudentInfo.action',
					type : 'POST',
					data : {
	...
	but3.click(function(){
		if (a.attr("name") === ""&&b.attr("name") === "") {
			...
			form.attr("action","teacherInit_outStudentExl.action");
			var input1 = $("<input>");
